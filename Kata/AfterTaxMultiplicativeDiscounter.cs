...
namespace Kata
{
    public class AfterTaxMultiplicativeDiscounter : IAfterTaxDiscount
    {
        private AfterTaxDefaultDiscounter _afterDefault;
        private AfterTaxUPCDiscounter _afterUPC;
        public AfterTaxMultiplicativeDiscounter(List<DiscountDetails> listOfDiscounts)
        {
        ...
        public double CalculateDiscountsAfter(Product product, double Price)
        {
            ...
            if (_afterUPC != null)
                upcAmount = _afterUPC.CalculateDiscountsAfter(product, Price-defaultAmount);
            return defaultAmount + upcAmount;
